...
def mixed_preferential_attachment(G_orig, m=3):
    ...
    for new_node in range(n0, n):
        ...
        attempts = 0
        max_attempts = 1000  # כדי לא להיתקע בלולאה אינסופית
...
def color_by_average_incoming_rating(G, threshold=2):
    for node in G.nodes():
        ...
        # הדפסה לצורך בדיקה (ניתן למחוק אם לא צריך)
        # print(f"Node {node}: avg rating = {avg_rating:.2f}, color = {color}")
...
def draw_Graph(G, title="Bit Coin"):
    ...
    nx.draw(G, pos, with_labels=False, node_color=colors, edge_color='gray', node_size=20, width=1)
...
# draw_Graph( G_mpa, "G_mpa")

# plot_directed_degree_distributions(max_connected_component_graph, title="original")
# plot_directed_degree_distributions(G_mpa, title="G_mpa")
